...
- [ ] `npm test` (Node.js client) passes locally
...
- [ ] Added a changeset (`npx changeset`) — required for any change under
      `src/`, `package.json`, `python/dalux_build/`, or `python/pyproject.toml`.
      Docs/CI-only changes can use `npx changeset add --empty`.
- [ ] Node.js (`src/`) and Python (`python/dalux_build/`) clients were updated
      together if this changes shared API behavior, or the gap is explained above.
- [ ] README / `python/README.md` updated if public behavior changed.
