...
> at your own risk. If you find any issues, please report them in the
> [GitHub issues](https://github.com/brunoadam/dalux-build/issues). It is
> suspected to have ground-breaking updates and backwards-incompatible changes
> in the near future, so please pin to a specific version if you want to avoid
> surprises.

API clients for the
[Dalux Build REST API](https://app.swaggerhub.com/apis-docs/Dalux/DaluxBuild-api/4.14),
available in two languages, plus a standalone webhook server built on the Python
client:
...
tested, and released together. [AGENTS.md](AGENTS.md) has pointers to the
canonical upstream Dalux Build API docs and where each client's API surface
lives in this repo.

> This API client is not affiliated with Dalux, and is not an official Dalux
> product. It is maintained by [Bruno Adam](https://github.com/brunoadam).
...
immediately before anything is allowed to publish). For the commands to run each
locally, see [javascript/README.md#testing](javascript/README.md#testing),
...
manual version bump, and nothing publishes unless the full test suite above
passes. See [CONTRIBUTING.md](CONTRIBUTING.md#how-releases-work) for the
complete flow, including how to add a changeset.
