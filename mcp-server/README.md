...
`download_file` downloads a file into a local cache directory (`$TMPDIR/dalux-mcp/files/<fileId>/`) *and* streams its content back in the tool result as a base64 embedded resource, up to an inline size limit (10 MB by default, `DALUX_MCP_MAX_INLINE_BYTES` to change the server-wide default) — past that limit it falls back to reporting the local path only, same as before, since inlining an arbitrarily large file would blow an LLM's context. A caller can also pass `maxInlineBytes` to raise the cap for one call, up to a hard ceiling of 500 MB (524288000 bytes); this is meant for when the user explicitly asks for a large file to be streamed back, not as a default. The local cache write happens either way, since `search_file_content`/`render_pdf_page`/the index builders read from it. `download_task_attachment` behaves the same way.
