 ...
 *
 * Override with DALUX_MCP_MAX_INLINE_BYTES for a deployment that wants a
 * different limit.
 */
...
export function maxInlineBytes(): number {
  ...
  return Number.isFinite(parsed) && parsed > 0 ? parsed : DEFAULT_MAX_INLINE_BYTES;
}
 ...
 */
export async function buildInlineResource(filePath: string, fileName: string): Promise<InlineResult> {
  let size: number;
  try {
