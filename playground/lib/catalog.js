...
// Python client exposes. Methods flagged `download: true` stream their
// result to the browser as a file attachment (see app/api/call/route.js)
// instead of the usual JSON response. Methods that need stdin, interactive
// file selection, or chunked uploads are still omitted because they don't
// make sense over a stateless HTTP console — they block on a terminal
// prompt or require multi-step client-side chunking, not a single request.
// Bulk download helpers (which write many files to the *server's* local
// disk) are also omitted for the same reason single-file downloads used to
// be: not something a stateless browser call fits into.
...
export const catalog = {
  ...
  files: {
    ...
    methods: {
      ...
      downloadFile: { write: false, http: 'GET', download: true, desc: 'Download a file\'s binary content to your browser.', params: [P.projectId, { name: 'fileAreaId', type: 'string', required: true }, { name: 'fileId', type: 'string', required: true }] },
  ...
  fileRevisions: {
    ...
    methods: {
      downloadFileRevision: { write: false, http: 'GET', download: true, desc: 'Download the binary content of a specific file revision to your browser.', params: [P.projectId, { name: 'fileAreaId', type: 'string', required: true }, { name: 'fileId', type: 'string', required: true }, { name: 'fileRevisionId', type: 'string', required: true }] },
    },
  },
