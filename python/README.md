...
Most methods take `project_id` (and, where relevant, `file_area_id`) as a **keyword-only**
argument. If you mostly work against a single project, set a default once on the client and
...
An explicit `project_id`/`file_area_id` passed to a call always overrides the client
default; a `ValidationError` is raised if neither is available.

### `full_response`

List/collection methods (`list_projects`, `get_project_tasks`, `list_files`, …) default to
returning just the plain `list[...]` of items. Pass `full_response=True` to get the full
...
`bulk_download_files`' own `file_area_id` parameter is the exception: passing `None` there
selects path-based resolution and is intentionally **not** backfilled from the client default.
