...
class TestPaginationUtils:
    ...
    def test_get_next_page_params(self):
        response = {"links": [{"rel": "nextPage", "href": "http://example.com?bookmark=abc123"}]}
...
class TestSearchUtils:
    ...
    def test_find_by_field(self):
        items = [{"id": 1, "name": "Alice"}, {"id": 2, "name": "Bob"}, {"id": 3, "name": "Charlie"}]
...
def test_get_sends_api_key_header():
    rsps_lib.add(rsps_lib.GET, f"{BASE_URL}/5.1/projects", json={"items": []}, status=200)
    config = Configuration(base_url=BASE_URL, api_key=API_KEY)
...
def test_get_sends_query_params():
    rsps_lib.add(rsps_lib.GET, f"{BASE_URL}/5.1/projects", json={}, status=200)
    config = Configuration(base_url=BASE_URL, api_key=API_KEY)
    client = ApiClient(config)
...
def test_post_sends_json_body():
    rsps_lib.add(rsps_lib.POST, f"{BASE_URL}/5.0/projects", json={"id": "p1"}, status=201)
    config = Configuration(base_url=BASE_URL, api_key=API_KEY)
    client = ApiClient(config)
...
def test_patch_sends_json_body():
    rsps_lib.add(rsps_lib.PATCH, f"{BASE_URL}/5.0/projects/p1", json={"id": "p1"}, status=200)
    config = Configuration(base_url=BASE_URL, api_key=API_KEY)
    client = ApiClient(config)
